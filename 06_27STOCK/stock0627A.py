...
# 臺股權值股與熱門股清單（用於相關係數比較）
_COMPARE_STOCKS = [
    '2330', '2317', '2454', '2308', '2881', '2882', '2891', '2303',
    '3711', '2886', '1301', '1303', '2002', '1326', '2912', '5880',
    '2884', '2885', '3045', '2357', '2382', '6505', '1216', '2345',
    '3008', '2327', '4904', '2395', '5871', '2892', '1101', '2207',
    '3034', '2379', '6669', '2603', '4938', '1590', '2049', '8046'
]

def calc_correlation(target_symbol, start_date=None, end_date=None):
    """計算目標股票與其他股票的日報酬率相關係數，回傳排序結果"""
    # 下載目標股票資料
    _, df_target, _ = fetch_data(target_symbol, start_date, end_date)
    if df_target.empty:
        return []
    ret_target = df_target['Close'].pct_change().dropna()

    results = []
    for code in _COMPARE_STOCKS:
        code_clean = code.split('.')[0]
        if code_clean == target_symbol.split('.')[0]:
            continue
        try:
            sym_cand = [f'{code}.TW', f'{code}.TWO']
            for sym in sym_cand:
                df_c, _, _ = fetch_data(sym, start_date, end_date)
                if not df_c.empty:
                    ret_c = df_c['Close'].pct_change().dropna()
                    # 取交集日期
                    common = ret_target.index.intersection(ret_c.index)
                    if len(common) < 30:
                        continue
                    corr = ret_target.loc[common].corr(ret_c.loc[common])
                    name = fetch_chinese_name(sym)
                    results.append((sym, name, corr))
                    break
        except Exception:
            continue
...
class StockApp:
    ...
    def worker(self, query, start_date, end_date):
        ...
        try:
            ...
            # 背景計算相關係數
            threading.Thread(target=self.corr_worker, args=(symbol, start_date, end_date), daemon=True).start()
        except Exception as e:
            self.root.after(0, self.show_error, str(e))

    def corr_worker(self, symbol, start_date, end_date):
        """背景計算日報酬率相關係數"""
        self.root.after(0, self.corr_text.set, "正在計算相關係數，請稍候...")
        try:
            results = calc_correlation(symbol, start_date, end_date)
            self.root.after(0, self.show_correlation, symbol, results)
        except Exception as e:
            self.root.after(0, self.corr_text.set, f"計算失敗: {e}")
    ...
    def show_correlation(self, symbol, results):
        ...
        if not results:
            ...
            return
        code = symbol.split('.')[0]
        lines = [f"與 {code} 日報酬率相關度最高的股票：\n"]
        for i, (sym, name, corr) in enumerate(results, 1):
            s = sym.split('.')[0]
            n = name if name else s
            bar_len = int(abs(corr) * 20)
            bar = '█' * bar_len + '░' * (20 - bar_len)
            lines.append(f"  {i}. {n}({s})  相關係數: {corr:+.4f}  {bar}")
        self.corr_text.set('\n'.join(lines))
