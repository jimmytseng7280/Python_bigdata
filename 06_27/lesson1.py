...
def challenge():
    ...
    plt.subplot(2, 2, 1)
    plt.plot(x, np.sin(np.pi * x), 'g-', linewidth=2)
    plt.title('sin(πx)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.4)

    plt.subplot(2, 2, 2)
    plt.plot(x, np.cos(np.pi * x), 'b--', linewidth=2)
    plt.title('cos(πx)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.4)

    plt.subplot(2, 2, 3)
    plt.plot(x, np.sin(2 * np.pi * x), 'r-.', linewidth=2)
    plt.title('sin(2πx)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.4)

    plt.subplot(2, 2, 4)
    plt.plot(x, np.cos(2 * np.pi * x), 'm:', linewidth=2)
    plt.title('cos(2πx)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.4)

    plt.suptitle('延伸挑戰：2×2 子圖', fontsize=16)
    plt.tight_layout()
    plt.show()
