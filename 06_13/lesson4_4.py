...
class ScoreDistributionApp:
	def __init__(self, root: tk.Tk) -> None:
		...
		self.rank_count_var = tk.StringVar(value="3")
		self.top_title_var = tk.StringVar(value="前 3 名")
		self.bottom_title_var = tk.StringVar(value="後 3 名")
	...
	def _build_ui(self) -> None:
		...
		control_row = tk.Frame(rank_card, bg=self.colors["panel"])
		...
		rank_entry = ttk.Entry(
			control_row,
			textvariable=self.rank_count_var,
			width=6,
			font=("PingFang TC", 11),
		)
		rank_entry.pack(side=tk.LEFT, padx=(6, 0))
		rank_entry.bind("<Return>", lambda _event: self.update_dashboard())
		tk.Button(
			control_row,
			text="更新",
		...
		tk.Label(
			rank_card,
			textvariable=self.bottom_title_var,
			bg=self.colors["panel"],
			fg="#ef4444",
			font=("PingFang TC", 11, "bold"),
		).pack(anchor="w")
	...
	def _get_rank_count(self) -> int:
		try:
			count = int(self.rank_count_var.get().strip())
		except ValueError:
			count = 3
		return max(1, count)

	def _update_rank_text(self, series: pd.Series) -> None:
		count = self._get_rank_count()
		self.top_title_var.set(f"前 {count} 名")
		self.bottom_title_var.set(f"後 {count} 名")

		ranking = pd.DataFrame({"學生姓名": self.df["學生姓名"], "score": series})
		top_rank = ranking.sort_values("score", ascending=False).head(count)
		bottom_rank = ranking.sort_values("score", ascending=True).head(count)

		top_lines = [f"{idx + 1}. {row['學生姓名']}  {row['score']:.1f}" for idx, row in top_rank.reset_index(drop=True).iterrows()]
		bottom_lines = [
			f"{idx + 1}. {row['學生姓名']}  {row['score']:.1f}" for idx, row in bottom_rank.reset_index(drop=True).iterrows()
		]
