...
CSV_CANDIDATES = [
    BASE_DIR / "考試分數_3年6班.csv",
    BASE_DIR / "考試分數_3 年 6 班.csv",
]
CSV_FILE = next((path for path in CSV_CANDIDATES if path.exists()), None)
if CSV_FILE is None:
    raise FileNotFoundError(f"找不到成績檔案。請確認檔案存在於：{BASE_DIR}")
...
def plot_student(name):
    ...
    if result.empty:
        clear_plot_area()
        messagebox.showinfo("結果", f"找不到學生：{name}")
        return
...
def show_personal_chart():
    name = entry_name.get().strip()
    if not name:
        messagebox.showwarning("提醒", "請先輸入學生姓名")
        return
    plot_student(name)


btn_plot = tk.Button(frame1, text="個人圖表", command=show_personal_chart, width=12,
