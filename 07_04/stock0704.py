...
def calc_correlation(target_symbol, start_date=None, end_date=None):
    ...
    參考 top5_corr.py：一次批次下載所有股票，再計算相關矩陣"""
    import warnings
    import numpy as _np
    warnings.filterwarnings('ignore')

    # 產生全部股票代碼（上市 1000~2500、上櫃 6000~7000）
    all_tickers = ([f"{i:04d}.TW" for i in range(1000, 2500)]
                   + [f"{i:04d}.TWO" for i in range(6000, 7000)])

    # 確認目標股票在清單中，若不在就加入
    target = target_symbol if '.' in target_symbol else f"{target_symbol}.TW"
    ...
    try:
        ...
        if start_date and end_date:
            ...
            kwargs['period'] = '2y'
        data = yf.download(all_tickers, **kwargs)
    ...
    # 取得收盤價
    close = data.get('Close') if isinstance(data.columns, pd.MultiIndex) else data[['Close']]
    ...
    yield int(close.shape[1]), total, []

    # 計算日報酬率
    returns = close.pct_change().replace([_np.inf, -_np.inf], _np.nan).dropna(axis=1, how='all').dropna()
    ...
    for col in returns.columns:
        if col == target or col.split('.')[0] == target.split('.')[0]:
            target_col = col
            break
    ...
    # 計算目標與其他股票的相關係數
    target_ret = returns[target_col]
    results = []
    for col in returns.columns:
        if col == target_col:
            continue
        corr_val = target_ret.corr(returns[col])
        if pd.isna(corr_val):
            continue
        code = col.split('.')[0]
