...
pg.setConfigOption('background', '#fff5f7')  # 設定圖表背景色為淺玫瑰粉
pg.setConfigOption('foreground', '#4a3040')  # 設定圖表前景色為深玫瑰棕
pg.setConfigOptions(antialias=True)          # 預設啟用抗鋸齒以提供平滑線條

# 柔美淺粉紅 UI 樣式表 (CSS/QSS)
MODERN_STYLE = """
QMainWindow {
...
class PyQtGraphDemoApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.fps_display.setStyleSheet("color: #a78bfa; font-weight: bold;")
    ...
    def _create_tab_oscilloscope(self):
        ...
        self.coord_label2 = pg.TextItem(anchor=(1, 0), color='#a78bfa')
    ...
    def regenerate_zoom_data(self):
        ...
        # 繪製曲線
        self.p_overview.plot(self.zoom_data, pen=pg.mkPen('#d4a0c0', width=1))
        self.p_detail.plot(self.zoom_data, pen=pg.mkPen('#a78bfa', width=1.8))
    ...
    def regenerate_scatter_data(self):
        ...
        self.scatter_item = pg.ScatterPlotItem(
            ...
            hoverPen=pg.mkPen('#be185d', width=1.5)
        )
